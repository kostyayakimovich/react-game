...
const getPlayerResults = (store) => (next) => async (action) => {
  if (action.type === INIT_RESULTS) {
    ...
    const {
      timerReducer: { results, time },
    } = store.getState();
    const newResults = [...results, { result: time, cardsAmount: action.data }]
      .sort((a, b) => a.result - b.result)
      .slice(0, 10);
    //const playerFirstName = JSON.parse(localStorage.getItem('memoryGamePlayer'))
    //   .firstName;
    // const playerLastName = JSON.parse(localStorage.getItem('memoryGamePlayer'))
    //   .lastName;
    //const player = `${playerFirstName} ${playerLastName}`;
    //console.log(action.data * 2, time, player);

    localStorage.setItem('memoryGameResults', JSON.stringify(newResults));
    store.dispatch({
      type: GET_RESULTS_SUCESS,
      data: newResults,
      resultTime: time,
